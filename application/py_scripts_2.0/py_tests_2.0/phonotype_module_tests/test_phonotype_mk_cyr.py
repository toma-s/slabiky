...
class TestSlPhonotypeModule(unittest.TestCase):
    ...
    def test_црн(self):
        word = run_through_module(Text('црн'))
        self.assertEqual(word.get_text(), 'црн')
        self.assertEqual(word.get_phonotypes(), [OBSTR, VOWEL, NASAL])

    def test_рж(self):
        word = run_through_module(Text('рж'))
        self.assertEqual(word.get_text(), 'рж')
        self.assertEqual(word.get_phonotypes(), [LIQUID, OBSTR])
...
data = ConfigData(path_to_scripts + '\configs\conf_mk_cyr.json')
