...
class TestSlPhonotypeModule(unittest.TestCase):
    ...
    def test_распаўсюджана(self):
        ...
        self.assertEqual(word.get_phonotypes(), [LIQUID, VOWEL, OBSTR, OBSTR, VOWEL, GLIDE, OBSTR, VOWEL, SUBUNIT, OBSTR, VOWEL, NASAL, VOWEL])

    def test_знаходзілася(self):
        word = run_through_module(Text('знаходзілася'))
        self.assertEqual(word.get_text(), 'знаходзілася')
        self.assertEqual(word.get_phonotypes(), [OBSTR, NASAL, VOWEL, OBSTR, VOWEL, SUBUNIT, OBSTR, VOWEL, LIQUID, VOWEL, OBSTR, VOWEL])
