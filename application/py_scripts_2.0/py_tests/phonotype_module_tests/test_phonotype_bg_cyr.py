...
class TestSlPhonotypeModule(unittest.TestCase):
    ...
    def test_странджа(self):
        word = run_through_module(Text('странджа'))
        self.assertEqual(word.get_text(), 'странджа')
        self.assertEqual(word.get_phonotypes(), [OBSTR, OBSTR, LIQUID, VOWEL, NASAL, SUBUNIT, OBSTR, VOWEL])
...
data = ConfigData(path_to_scripts + '\configs\conf_bg_cyr.json')
